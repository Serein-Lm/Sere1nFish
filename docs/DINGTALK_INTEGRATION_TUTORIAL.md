...
- 执行中只显示一行当前阶段，完成后自动隐藏执行状态。
- 移除移动端容易被压缩成黑块的百分比进度条。
- 状态行使用无背景的全宽自适应布局，不使用固定 `50 × 100` 网格。
- 移除当前未使用的图表渲染区和多余分隔线，优先展示回答正文。
- 关闭动画渐变边框和问题图标，避免持续闪烁并减少视觉噪音。
...
- 单聊按“机器人 + 钉钉会话”隔离。
- 群聊按“机器人 + 群 + 发言成员”隔离，同一群内不同成员不会互相继承上下文。
- MongoDB 保留当前会话的消息历史；模型每轮只读取最近 12 条，并受总字符数限制，不会把全部历史都发送给模型。

需要开始一个全新话题时，单独发送以下任一命令：
...
机器人会直接确认清空，不创建 AI Card，也不调用模型。该操作只清除当前成员在当前会话中的消息历史，不影响其他成员、其他群或单聊，也不删除已经生成的 Word、Markdown 等产物。清空时仍在执行的旧请求会被上下文版本校验拦截，其迟到回答不会重新写入新上下文。
