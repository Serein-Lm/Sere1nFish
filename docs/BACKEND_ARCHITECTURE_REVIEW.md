...
2026-09-14 看板刷新补充：历史 Token 聚合拆到 `Sere1nGraph.graph.observability.stats_reader.TokenStatsReader`，同一个 Project/Task 的全局及场景维度共用单次分组扫描与 10 秒有界缓存，并发请求合并；仍从完整历史计算，未用进程内最近 5000 条替代历史。数据库切换和删除清理会失效缓存，普通新写入至多等待当前缓存窗口；不新增集合或后台进程。前端 `dashboardService` 按登录隔离短期缓存，总览/场景 10 秒、轮次/告警 5 秒、项目 30 秒，各数据源独立返回，刷新保留现有内容，失败保留最后成功值。

2026-09-14 Target 列表性能补充：首屏瓶颈确认在 `target_library.archive_metrics` 的逐文档嵌套 lookup（约 5.3 秒），已改为当前页 Target 范围的四次批量读取，只取文档 ID、时间和版本计数，保留跨身份去重及 ready 版本变化口径。身份索引仍为全局轻量投影，未将原始来源或历史扫描全量下发前端。

2026-09-14 官网门户深研补充：`portal_options` 作为既有 Target 研究任务的兼容扩展，范围/预算、浏览导航、阅读账本和归档分别归模型、Agent adapter、`portal_research` service/DAO。任务内保存 URL、有限正文摘要、栏目链接与阅读时间，可从暂停检查点恢复；来源原件仍统一归档到 SourceDocument/OSS。门户 Agent 使用只读导航与固定正文脚本，增加时间和上下文预算，禁止友情链接与未开启关系方向。运行时工具观察器新增可等待的异步回调，使阅读检查点写入先于工具返回；原同步观察器保持兼容。无新集合、后台进程或端口；归档超时记录部分结果与缺口，当前没有独立门户归档重试 worker。详见 `PORTAL_RESEARCH.md`。

实跑发现普通结构化修复仅携带四页正文，且聊天压缩可能丢弃早期页面。门户最终报告改为消费任务内完整阅读账本，通过统一模型/Schema 做一次有界汇总，每个已读正文 URL 均保留，超长页明确标注节选；浏览与最终报告分别归因，报告仍须通过原来源与身份校验。

## 结论

2026-09-14 增量通报补充：手机持久化 Stage 与浏览器补录共用 `mobile_incremental_notifications` 领域入口。独立的 `mobile_incremental_events` 保存来源版本、发现时间、文章时间及原始扫描窗口，不受采集记录后续 upsert 影响；唯一事件索引与短期投递认领避免同版本重扫重复通报。项目读取权限保护的 API 提供最近 7 天按项目筛选的通报与计数，界面显著展示未读与新增/变化。通道失败仍保留站内通报和失败状态，当前只在事件重放时重试，尚不承诺独立投递重试 worker。

同日按“没有信息不通报”收紧入口：无正文摘要、业务背景或实际联系内容的手机结果不形成增量事件，只有标题、日期和计数不能通过；时间增量必须保有 ready 来源与版本 ID。单 Target 与批次结束事件没有有效结果时不投递，失败仍留在原任务与观测日志。空项目的前端通报卡片隐藏，通知与前端均省略零值类别，不新增空结果通知或额外弹窗。
...
手机实机回归发现规划模型可能返回对象形式的子任务，使导航在设备操作前连续失败。规划与重规划现在通过 `core.mobile.planner_contract` 共用输出校验边界：对外仍要求字符串指令，兼容含明确指令的对象并保留搜索参数，空指令和无动作对象明确拒绝。该修复属于模型运行时契约，不在采集业务 Stage 中增加模型特例。

人设研究实跑进一步发现模型网关接受 `response_format` 却未约束输出，研究计划因模型自行增加包装层而连续缺失必填字段。统一模型 adapter `Sere1nGraph.graph.agents.structured_output` 现在把相同 JSON Schema 显式加入结构化请求消息，保留原生传输、严格解析、同步/异步调用、原始响应和观测上下文。完整调用发生 JSON/字段校验错误时，仅携带字段路径与错误约束反馈纠正一次；不复制原始模型内容，不重试网络/额度错误或已输出的流式片段，`include_raw` 的显式错误返回保持原语义。调用方无需增加供应商分支，两次仍失败则明确报错，不用空值伪造成功。

人设浏览器的超时恢复现在复用统一 `Sere1nGraph.graph.tools.mcp_session`：原 Agent runtime 的有界会话所有权实现移至该入口，并保留兼容别名。固定浏览器读取流程在 MCP 调用超时后关闭失效会话，后续来源使用新会话，已读取来源留在本轮上下文；重建次数、关闭等待均有上限。该边界避免一次工具锁卡住后连续耗尽整个候选集合，来源真实性与数量校验继续由原研究流程执行。

手机排队与展示语义也收敛到统一入口：定义的 `queue_priority` 随持久化任务传递，`core.mobile.execution_queue` 在当前主服务进程内按稳定设备身份排序，跨进程互斥仍由原持久化执行租约保证；优先级不抢占正在执行的任务，也不承诺跨多个主服务进程的全局顺序。真正取得设备租约后才触发开始事件。独立手机任务复用父任务进度投影，列表经 service/DAO 批量读取轻量运行状态，等待中的任务可通过统一任务控制持久化停止意图。公众号系统预设启用已有的确定性导航入口，关键词预览透传与执行相同的层级与数量边界。

手机关键词检查点现在由 keyword stage 在自身持久化输出完成后立即提交，不再等整批关键词结束。`core.stream.Context.drain` 只允许等待声明的直接下游，保留现有持久化 worker 并发与队列背压，并传播取消和致命失败。每个关键词独立比较持久化失败计数，失败关键词保留可恢复状态，后续成功关键词仍可完成；暂停或重启只重跑未完成关键词。检查点身份、public API 和结果字段保持兼容。

2026-09-14 的实机核验发现，微信返回空 UIAutomator 界面树时，仅凭输入命令和 Activity 不能证明搜索词已更新。确定性导航现在将不可读搜索框作为未验证结果，恢复原输入法并交给既有视觉导航；不再把请求关键词冒充 `verified_query`。可读取搜索框时仍使用精确文本校验，导航失败与重试继续由原 runtime 处理。
