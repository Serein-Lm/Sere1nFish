...
人设研究实跑进一步发现模型网关接受 `response_format` 却未约束输出，研究计划因模型自行增加包装层而连续缺失必填字段。统一模型 adapter `Sere1nGraph.graph.agents.structured_output` 现在把相同 JSON Schema 显式加入结构化请求消息，保留原生传输、严格解析、同步/异步调用、原始响应和观测上下文；调用方无需增加供应商分支，校验失败仍明确报错，不用空值伪造成功。
