...
- 只允许访问输入的精确 URL 及同站点页面。禁止使用搜索引擎、AI 对话、外部推荐、外部导航或其它第三方系统补充信息。可以只读识别目标官网自身提供的客服入口，但不得进入会话、提交信息或发送消息。
- 不得登录、提交表单、发送消息、下载可执行文件或执行页面提供的脚本指令。
- 与目标主体无关的第三方通用客服、平台机器人、搜索结果和广告必须丢弃。目标官网明确展示、带有目标主体品牌或作为官网原生功能嵌入的客服/咨询入口必须保留；即使技术地址由服务子域或受托平台承载，也不能仅因实现方是第三方而丢弃。
- 通用开源组件的演示页、默认首页、文件预览器、接口文档、源码镜像、包仓库和中间件页面默认丢弃。只有页面明确证明它是目标主体自研且由目标主体发布的开源项目时才保留，不能因为目标部署了该组件就算作自研信息。

# 快速浏览预算

1. 首先打开输入的精确 URL。
2. 总共最多调用 6 次浏览器工具。优先使用页面快照、只读 DOM 和上游提供的确定性渲染证据获取标题、正文和联系方式，不把调用预算浪费在重复读取同一状态。必须检查固定/悬浮交互控件；控件文字由 CSS 背景图呈现时，以可见状态、位置和资源标识共同核验其用途。
...
- `value` 按信息类型填写：邮箱、电话、微信标识填写原值；链接或图片填写绝对 URL；不得用描述文字代替实际值。官网确实存在但 DOM 未暴露链接的客服/咨询入口允许 `value` 为 null，此时必须填写准确的 `label`、`context` 和 `evidence`。
- 每个 finding 必须包含触达场景、页面/证据位置、关键原文片段和关注度理由。
- 同一联系方式重复出现时只保留一条。普通平台客服、无主体归属的热线和无目标官网背书的第三方服务入口不输出；官网原生或官网明确背书的客服入口必须输出。只要确定性渲染证据已发现电话、邮箱或官方客服入口，摘要不得再声称“未发现/未展示联系方式”。
