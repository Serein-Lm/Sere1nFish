...
@tool(
    ...
    description=(
        "检查一个项目可读取的数据源及可用性。返回网站、资产、小红书、抖音、公众号原文、"
        "Target、手机画像/观察/截图元数据/操作/会话、学者、任务、日志、finding 和 AI 产物等数据源。"
        "引用整个项目或不确定应该读取哪个数据源时先调用。参数：project_id。"
    ),
)
...
@tool(
    ...
    description=(
        ...
        "参数：project_id、source、limit（默认20，上限50）。"
    ),
)
def read_project_dataset(project_id: str, source: str, limit: int = 20) -> str:
    project_id = str(project_id or "").strip()
