...
from core.mobile.execution_queue import PriorityExecutionQueue, queue_priority_value
from core.mobile.pool import DevicePool

logger = get_logger("mobile_device_leases")
_DEVICE_QUEUES: WeakKeyDictionary = WeakKeyDictionary()


def _device_queue(device_key: str) -> PriorityExecutionQueue:
    queues = _DEVICE_QUEUES.setdefault(asyncio.get_running_loop(), {})
    return queues.setdefault(device_key, PriorityExecutionQueue())
...
async def background_device_lease(
    ...
    *,
    ...
    async with _device_queue(device_key).slot(queue_priority_value(queue_priority)), mobile_execution_lease(
        db,
        device_id=device_id,
        task_id=run_task_id,
        owner=owner,
