...
def profile_ready(profile: dict) -> bool:
    return bool(len(str(profile.get("summary") or "")) >= 80 and profile.get("company") and profile.get("position")
        and ((profile.get("context_complete") and (profile.get("context_review") or {}).get("passed") and (profile.get("context_review") or {}).get("policy_version", 0) >= 2) or (profile.get("source_urls") and profile.get("research_evidence"))))
...
async def coverage(db) -> dict:
    ...
    return {"policy_version": 3, "default_generation_mode": "context", "standard": catalog()["standard"], "source_url": catalog()["source_url"], "sectors": [{**item, "person_count": sectors[item["code"]]} for item in catalog()["sectors"]], "items": rows,
