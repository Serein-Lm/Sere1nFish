...
JPEG_QUALITY = min(95, max(60, int(os.getenv("DEEPFAKE_JPEG_QUALITY", "88"))))
...
def _encode_jpeg(frame: numpy.ndarray[Any, Any]) -> bytes:
...
class FaceFusionRuntime:
    ...
    def __init__(self) -> None:
        ...
        self.model = ""
        self.pixel_boost = ""
        self._processor_modules: list[Any] = []
    ...
    def _initialize_sync(self) -> None:
        ...
        self._processor_modules = get_processors_modules(state_manager.get_item("processors"))
        self._warmup_sync()
    ...
    def _warmup_sync(self) -> None:
        ...
        self.process_sync([source], target)

    def validate_source_sync(self, source_frames: list[numpy.ndarray[Any, Any]]) -> None:
        from facefusion.face_creator import get_one_face, get_static_faces

        if not get_one_face(get_static_faces(source_frames)):
            raise GatewayError("No face was detected in the source image")

    async def validate_source(self, source_frames: list[numpy.ndarray[Any, Any]]) -> None:
        async with self._inference_lock:
            await asyncio.to_thread(self.validate_source_sync, source_frames)
    ...
    def process_sync(
        ...
        for processor in self._processor_modules:
    ...
    async def process(
        ...
        *,
        ...
        async with self._inference_lock:
            ...
            output = await asyncio.to_thread(self.process_sync, source_frames, target_frame)
...
app = FastAPI(title="Sere1nFish Deepfake Gateway", version="1.0.0", lifespan=lifespan)
...
async def swap_image(
    ...
    source_frame = _decode_image(await source.read(MAX_IMAGE_BYTES + 1), label="source")
    target_frame = _fit_frame(
        _decode_image(await target.read(MAX_IMAGE_BYTES + 1), label="target"),
        max_width,
    )
    try:
        await runtime.validate_source([source_frame])
        output, inference_ms = await runtime.process([source_frame], target_frame, analyse_content=True)
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                await websocket.send_bytes(_encode_jpeg(output))
