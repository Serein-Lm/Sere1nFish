...
class ScrcpyStreamer:
    ...
    def __init__(
        ...
        Args:
            ...
            max_fps: Maximum encoded frames per second
    ...
    def _build_server_options(self) -> ScrcpyServerOptions:
        ...
        return ScrcpyServerOptions(
            ...
            downsize_on_error=self.downsize_on_error,
            tunnel_forward=True,
            audio=False,
            control=False,
            cleanup=False,
    ...
    async def _start_server(self) -> None:
        ...
        for attempt in range(max_retries):
            ...
            server_args = [
                ...
                f"downsize_on_error={str(options.downsize_on_error).lower()}",
    ...
    async def _read_exactly(self, size: int) -> bytes:
        ...
        while len(self._read_buffer) < size:
            chunk = await asyncio.to_thread(
                self.tcp_socket.recv, max(65536, size - len(self._read_buffer))
            )
            if not chunk:
