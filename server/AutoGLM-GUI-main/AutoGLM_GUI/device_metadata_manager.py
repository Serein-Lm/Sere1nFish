...
class DeviceMetadataManager:
    ...
    def delete_metadata(self, serial: str) -> bool:
        """Delete all persisted metadata for a device."""
        with self._data_lock:
            if serial not in self._metadata:
                return False

            self._metadata.pop(serial, None)
            self._save_metadata()

        logger.info(f"Deleted metadata for device {serial}")
        return True
