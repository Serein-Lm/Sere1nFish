...
class ManagedDevice:
    ...
    def status(self) -> str:
        ...
        if self.state == DeviceState.DISCONNECTED:
            return "disconnected"
        if self.state == DeviceState.AVAILABLE_MDNS:
            return "available"
        if not self.connections:
            return "unknown"
    ...
    def online(self) -> bool:
        """Whether the device is currently usable for operations."""
        return (
            self.state == DeviceState.ONLINE
            and self.primary_connection.status == "device"
        )
...
class DeviceManager:
    ...
    def remove_device(self, serial: str, *, disconnect: bool = True) -> tuple[bool, str, bool]:
        """Forget a device and optionally disconnect its active transport.

        For WiFi ADB devices this runs ``adb disconnect`` first. USB devices cannot
        be physically disconnected by software; they are removed from the current
        cache and will reappear on the next poll if still plugged in.

        Returns:
            Tuple of (success, message, disconnected).
        """
        disconnected = False

        with self._devices_lock:
            managed = self._devices.get(serial)
            mdns_managed = self._mdns_devices.get(serial)

            if not managed and not mdns_managed:
                self._metadata_manager.delete_metadata(serial)
                return (False, "Device not found", disconnected)

            target = managed or mdns_managed
            if target and disconnect:
                if target.connection_type == DeviceConnectionType.REMOTE:
                    ok, msg = self.remove_remote_device(serial)
                    if ok:
                        self._metadata_manager.delete_metadata(serial)
                    return (ok, msg, ok)

                if target.connection_type == DeviceConnectionType.WIFI:
                    ok, msg = self.disconnect_wifi(target.primary_device_id)
                    disconnected = ok
                    if not ok:
                        logger.warning(
                            f"Failed to disconnect WiFi before deleting {serial}: {msg}"
                        )

            managed = self._devices.pop(serial, None)
            self._mdns_devices.pop(serial, None)
            self._remote_devices.pop(serial, None)
            self._remote_device_configs.pop(serial, None)
            if managed:
                for conn in managed.connections:
                    self._device_id_to_serial.pop(conn.device_id, None)

            self._adb_keyboard_attempted_serials.discard(serial)
            self._adb_keyboard_ready_serials.discard(serial)
            self._metadata_manager.delete_metadata(serial)

        logger.info(f"Device removed from cache: {serial}")
        return (True, "Device removed", disconnected)
    ...
    def get_device_protocol(self, device_id: ConnectionDeviceID) -> DeviceProtocol:
        ...
        with self._devices_lock:
            ...
            if not managed.online:
                raise ValueError(
                    f"Device {device_id} is not online (state={managed.state.value}, "
