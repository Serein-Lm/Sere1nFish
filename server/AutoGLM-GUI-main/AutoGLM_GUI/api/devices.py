...
def _list_devices_sync() -> DeviceListResponse:
...
@router.delete("/api/devices/{serial}", response_model=DeviceDeleteResponse)
async def delete_device(serial: str) -> DeviceDeleteResponse:
    """Remove a device from the local inventory and clear its metadata."""
    from AutoGLM_GUI.device_group_manager import device_group_manager
    from AutoGLM_GUI.device_manager import DeviceManager
    from AutoGLM_GUI.phone_agent_manager import PhoneAgentManager
    ...
    device_ids = [conn.device_id for conn in managed.connections] if managed else []

    for device_id in device_ids or [serial]:
        stop_streamers(device_id=device_id)
        try:
            PhoneAgentManager.get_instance().destroy_agents_for_device(device_id)
        except Exception as exc:  # noqa: BLE001
            logger.warning(f"Failed to destroy agents for {device_id}: {exc}")

    success, message, disconnected = device_manager.remove_device(serial)
    device_group_manager.unassign_device(serial)
