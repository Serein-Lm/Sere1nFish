...
所有接口统一错误结构，**本轮起附带 `path`**（便于前端定位出错端点）：

- 业务 / HTTP 错误：`{ "detail": "...", "path": "/api/v1/..." }`
- 参数校验错（422）：`{ "detail": [ { "loc": [...], "msg": "...", "type": "..." } ], "path": "..." }`
- 服务器内部错（500）：`{ "detail": "服务器内部错误", "path": "..." }`（堆栈仅入服务端日志，不外泄）

健康检查 `GET /health` 现返回 `{ "status": "ok" | "degraded", "mongodb": { "ok": bool, "error"? } }`（附带 MongoDB 连通性探测，2s 超时）。

> 说明：`POST /tasks` 批量删除（`DELETE /projects/{id}/tasks`）响应新增 `deleted_findings` / `deleted_copywritings` 计数（附加字段，向后兼容）。
