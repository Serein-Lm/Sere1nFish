...
# 延迟导入 AsyncWordCloudGenerator
def _get_wordcloud_generator():
    """Lazy load AsyncWordCloudGenerator"""
    if not config.ENABLE_GET_WORDCLOUD:
        return None
    try:
        from tools.words import AsyncWordCloudGenerator
        return AsyncWordCloudGenerator()
    except ImportError:
        utils.logger.warning("[AsyncFileWriter] WordCloud dependencies not installed, skipping wordcloud generation")
        return None
...
def _check_aiofiles():
    if not _AIOFILES_AVAILABLE:
        raise ImportError(
            "aiofiles is required for async file operations. Install with: pip install aiofiles"
        )
