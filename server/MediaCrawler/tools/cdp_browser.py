...
class CDPBrowserManager:
    ...
    def __init__(self):
        ...
        self._remote_task_id: Optional[str] = None  # Docker 模式下的 task_id
    ...
    async def launch_and_connect(
        ...
        支持两种模式（透明切换）：
        - Docker 模式：通过 BrowserProvider 获取远程 Chrome 容器的 CDP 地址
        - 本地模式：走原有的本地 Chrome 启动逻辑
        """
        try:
            # 尝试通过 BrowserProvider 获取远程 CDP 地址
            remote_ws_url = await self._try_get_remote_cdp(task_id, purpose="xhs")

            if remote_ws_url:
                # ── Docker 模式：连接远程 Chrome 容器 ──
                utils.logger.info(f"[CDPBrowserManager] Docker 模式: 连接远程 Chrome → {remote_ws_url}")
                try:
                    self.browser = await playwright.chromium.connect_over_cdp(remote_ws_url)
                    utils.logger.info(
                        f"[CDPBrowserManager] Docker Chrome 连接成功 | "
                        f"version={self.browser.version} | "
                        f"contexts={len(self.browser.contexts)}"
                    )
                except Exception as e:
                    utils.logger.error(
                        f"[CDPBrowserManager] Docker Chrome 连接失败 | "
                        f"ws_url={remote_ws_url} | error={e}"
                    )
                    raise
                self._remote_task_id = task_id  # 记录 task_id 用于 cleanup 时释放
            else:
                # ── 本地模式：走原有逻辑 ──
    ...
    async def _try_get_remote_cdp(self, task_id: Optional[str] = None, purpose: str = "general") -> Optional[str]:
        """
        尝试从 BrowserProvider 获取远程 CDP 地址。
        返回 None 表示走本地模式。
        """
        try:
            ...
            utils.logger.info(
                f"[CDPBrowserManager] 尝试获取远程 CDP | "
                f"provider={type(provider).__name__} | task_id={task_id} | purpose={purpose}"
            )
            endpoint = await provider.get_cdp_endpoint(task_id=task_id, purpose=purpose)
            if endpoint:
                utils.logger.info(f"[CDPBrowserManager] 远程 CDP 端点获取成功: {endpoint}")
            else:
                utils.logger.info("[CDPBrowserManager] Provider 返回 None，将使用本地模式")
            return endpoint
        except ImportError:
            utils.logger.debug("[CDPBrowserManager] browser_manager 模块未安装，走本地模式")
            return None
        except Exception as e:
            utils.logger.warning(f"[CDPBrowserManager] BrowserProvider 异常，走本地模式: {e}")
            return None
    ...
    async def cleanup(self, force: bool = False):
        ...
        """
        try:
            # 释放 Docker 容器（如果是 Docker 模式）
            if self._remote_task_id:
                try:
                    from browser_manager import get_browser_provider
                    provider = get_browser_provider()
                    await provider.release_cdp_endpoint(task_id=self._remote_task_id)
                    utils.logger.info(f"[CDPBrowserManager] Released Docker container (task={self._remote_task_id})")
                except Exception as e:
                    utils.logger.warning(f"[CDPBrowserManager] Failed to release Docker container: {e}")
                finally:
