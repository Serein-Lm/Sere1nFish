...
class RedisCache(AbstractCache):
    ...
    def __init__(self) -> None:
        # 延迟导入 redis，避免在不使用时也要求安装
        try:
            from redis import Redis
            # Connect to redis, return redis client
            self._redis_client = self._connet_redis(Redis)
        except ImportError as e:
            raise ImportError(
                "Redis module not installed. Please install it with: pip install redis"
            ) from e

    @staticmethod
    def _connet_redis(Redis) -> Any:
        """
