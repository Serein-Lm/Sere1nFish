...
class XiaoHongShuClient(AbstractApiClient, ProxyRefreshMixin):
    ...
    async def request(self, method, url, **kwargs) -> Union[str, Any]:
        ...
        # 详细日志：查看响应内容
        utils.logger.info(f"[XiaoHongShuClient.request] Status: {response.status_code}")
        utils.logger.info(f"[XiaoHongShuClient.request] Response text (first 500): {response.text[:500]}")
    ...
    async def pong(self) -> bool:
        ...
        utils.logger.info(f"[XiaoHongShuClient.pong] a1 value: {self.cookie_dict.get('a1', 'NOT FOUND')[:20] if self.cookie_dict.get('a1') else 'NOT FOUND'}...")
        ping_flag = False
        try:
            note_card: Dict = await self.get_note_by_keyword(keyword="Xiaohongshu")
            utils.logger.info(f"[XiaoHongShuClient.pong] Response keys: {list(note_card.keys()) if note_card else 'None'}")
