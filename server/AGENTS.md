...
- `utils` 只容纳无领域状态、可独立测试、可跨模块复用的纯工具。带数据库、网络、配置、权限或业务状态的逻辑属于 DAO/service/adapter，不得把业务耦合转移到万能 `utils.py`。
- JSON、文件上传、AI 工具、定时任务和外部渠道触发同一领域动作时，必须汇入同一个 service 入口；不得在 Router 或调用侧复制校验、持久化和执行逻辑。
- 架构拆分按“契约测试 -> 接口/DAO -> 调用迁移 -> 删除旧实现”分批提交，保持行为兼容和可回滚。

## 分布式扫描演进约束

- 分布式扫描采用中心控制面和无状态执行节点。执行节点通过能力声明领取短租约任务，不直接访问中心 MongoDB/Redis，也不持有长期业务密钥。
- 任务必须具备稳定工作项 ID、原子认领、租约续期、幂等提交、取消传播和失联恢复；节点、浏览器和代理选择收敛到 registry/factory，业务任务不得写死地址或凭据。
- 节点只主动出站连接中心 HTTPS/WSS `443`，不向公网开放数据库、CDP、VNC、ADB 或 worker 端口。节点身份使用一次性注册凭据与可轮换 mTLS/签名身份。
- 远程产物通过统一对象存储协议和短时 STS/预签名上传；任务消息只传对象引用与校验值，不传大文件，不向节点下发 OSS AK/SK。
- SOCKS5 通过独立 `ProxyProvider` 接入，支持加密凭据、健康、容量、冷却、Target 粘性和显式 DNS/bypass 策略。要求代理的任务不得静默直连回退，代理失败必须与目标站失败、节点失败分开观测。
- 具体协议、数据模型、安全边界和分阶段迁移遵循 `../docs/DISTRIBUTED_SCAN_NODES.md`。
...
- 后端和前端是独立源码仓库时，除部署相关内容外，后端改动只在后端仓库提交，前端改动只在前端仓库提交。
