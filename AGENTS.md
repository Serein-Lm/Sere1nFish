...
- 官网门户深研通过既有 `target_research` 任务的可选 `portal_options` 接入 `api.services.portal_research`，浏览策略由专用 Agent factory/adapter 执行，阅读摘要与链接地图存入 `tasks.checkpoint.portal_research`，不复制原始 HTML。默认研究 60 分钟、50 页、120 次工具调用、24K 上下文，直属下级默认开启，直接上级可选；友情链接、合作伙伴与普通外部推荐不得作为单位扩展路径。六类栏目报告与 SourceDocument 版本引用共存于 `target_research` 历史；扩展后仅继承官网深采，不自动开启手机、招投标或话术渠道。边界及恢复语义见 `docs/PORTAL_RESEARCH.md`。
...
- 公众号深采采用“手机发现、浏览器读取”的职责划分：手机只负责应用内搜索、命中文章和复制真实链接；链接交给 `api.services.source_documents` 的 Provider registry，由项目 Chrome 池读取全文和媒体。浏览器读取失败时，必须先把 `source_archive_status/error/next_retry_at` 写入手机采集记录，再由 `api.services.mobile_source_handoff` 使用持久化租约独立补录；额度、限流、浏览器故障和进程重启都不得丢失链接或要求手机重复滚动。只有明确允许回退的非公众号策略才能继续手机逐屏深采。公众号候选默认按标题或账号直接身份对应做严格审核，多主体合集降主体分；仅当搜索词本身含投稿/征稿/约稿意图时按 `WechatArticleCandidatePolicy` 的投稿分支放宽：合集文章可入库（主体分门槛 40、点击前后一致即可），目标的投稿渠道常藏在合集正文中，其余意图保持严格门控。来源归档分析与独立复审同样遵循该例外：投稿意图下 `multi_entity_roundup` 的主体分上限放宽到 69（`_SUBMISSION_ROUNDUP_CAP`），目标在合集中拥有具体投稿条目即按 40-69 评分，`source_document` 两份提示词与 `is_submission_intent` 是同一套判定，不得只放宽其中一侧。
...
- 来源版本层只保存文章自身事实和证据；ProjectTarget 关联层保存搜索场景和任务分析；手机采集记录保存本次任务结果；前端按项目过滤记录并可按 Target 聚合。按 Target 读取手机记录和 wechat_count 时，范围是「该 Target 及其项目内下属单位」（`project_target_scope_ids` 与 `_rolled_scope_counts` 同一套层级判定），集团视图必须包含下属记录；叶子单位仍只含自身。禁止在这些层之间复制原始 HTML 或把搜索关键词自动当作公司名。

## 分层规范与记录

- 手机增量通报通过 `api.services.mobile_incremental_notifications` 统一发布，`mobile_incremental_events` 按 Project、Target、来源及版本保存不可覆盖的事件，实时采集与浏览器补录共用时间窗口核验和幂等身份。增量主动通报以钉钉为唯一主动渠道，前端不再展示增量信息（未读角标、项目通报卡片与历史抽屉已移除，不得在前端重新引入自动弹窗或角标）；通知渠道仍通过 `notifications.notify_event`，不得把未归档、日期未知或窗口外的记录通报为已确认时间增量。
- 没有实际采集内容不通报：手机记录只有标题、日期、计数或“暂无信息”等占位内容时，不创建增量事件、不发钉钉；时间增量必须已有 ready 来源身份及版本。没有新增/变化的项目不通报，零值类别不突出展示。公司完成或失败汇总没有有效结果时也保持静默，任务状态、错误和恢复日志仍持久化；系统级容量与基础设施告警沿用独立事件策略。
...
- 同一种解析、重试、限流、分页、下载、对象存储、模型调用、浏览器会话或设备动作只能有一个权威入口。新增实现前先用 `rg` 检索现有能力；若语义相同则扩展协议/registry，若语义不同则明确命名差异并补契约测试。
- Router 与页面不得通过复制 service 代码获得“方便”。HTTP/React 层只组装输入输出；JSON、文件上传、AI 工具、定时任务和外部渠道触发同一领域动作时，必须汇入同一个 service 入口。
- 架构拆分必须保持行为兼容并分批提交：先加契约测试，再抽接口/DAO，随后迁移调用方，最后删除旧实现。禁止把目录搬迁、行为修改和数据迁移混在一个无法回滚的大提交中。
...
- 扫描任务模板由 `api.services.scan_templates` 统一管理（system_config 的 `scan_templates` 加密段，admin 经 `/api/v1/config/scan-templates` 增删改查）：模板只保存非目标类参数（渠道开关、深度、并发），`params` 白名单在 `_ALLOWED_PARAM_KEYS`；公司列表、URL、批次标签等每次下发的目标字段不进模板。前端下发弹窗在表单挂载后（`afterOpenChange`，destroyOnHidden 模式下字段未挂载时 `setFieldsValue` 会静默丢失）自动套用默认模板，模板选择只是预设表单值、不改变下发请求结构。任务 Skills 默认不选 = 全部可用（自动路由），选择才限定范围。
