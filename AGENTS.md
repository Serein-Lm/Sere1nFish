...
- 没有实际采集内容不通报：手机记录只有标题、日期、计数或“暂无信息”等占位内容时，不创建增量事件、不发钉钉；时间增量必须已有 ready 来源身份及版本。没有新增/变化的项目不显示通报卡片，零值类别不突出展示。公司完成或失败汇总没有有效结果时也保持静默，任务状态、错误和恢复日志仍持久化；系统级容量与基础设施告警沿用独立事件策略。
